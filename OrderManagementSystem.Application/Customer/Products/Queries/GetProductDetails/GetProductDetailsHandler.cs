...
{
    public class GetProductDetailsHandler : IRequestHandler<GetProductDetailsQuery, ResponseDto<ProductDetailsDto>>
    {
        ...
        public async Task<ResponseDto<ProductDetailsDto>> Handle(GetProductDetailsQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (product == null)
                {
                    ...
                    return ResponseDto<ProductDetailsDto>.Error(ErrorCode.NotFound, $" Product with ID {request.Id} not found");
                }

                _logger.LogInformation("Product with ID {ProductId} retrieved successfully", request.Id);
                return ResponseDto<ProductDetailsDto>.Success(product, $"Product with id {request.Id} retrieved successfully");

            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to retrieve product with ID {ProductId}", request.Id);
                return ResponseDto<ProductDetailsDto>.Error(ErrorCode.DatabaseError, "Failed to retrieve product.");
            }
        }
    }
}